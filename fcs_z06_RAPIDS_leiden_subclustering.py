...
def rapids_leiden_clustering(cl, edgelist_file, rp_list, save_dir, label):     
    ...
    for rp in rp_list:
        ...
        partition.to_csv(f'{save_dir}/leiden_cluster_{cl}_subclstr_rp_{rp}_{label}.csv.gz', index=False, compression="gzip")
...
if __name__ == "__main__":
    ...
    with open('config.yaml', 'r') as f:
        ...
        cluster_list = config['z06']['cluster_list']
        label = config['z06']['label']
        edgelist_loc = config['z06']['edgelist_loc']
        rp_pick = config['z06']['rp_pick']
        rp_list = eval(config['z06']['rp_list'])
        save_dir = config['z06']['save_dir']
 
    print(config['z06'])

    edgelist_loc = f'{edgelist_loc}_rp_pick{rp_pick}'
    print(edgelist_loc)
    
    for cl in  cluster_list:
        edgelist_file = f'{edgelist_loc}/edgelist_cluster_{cl}.csv.gz'
        print(edgelist_file)
        
        save_dir_updated = f'z06_{save_dir}/cluster_{cl}_subclustering'
        print(save_dir_updated)

        rapids_leiden_clustering(cl, edgelist_file, rp_list, save_dir_updated, label)
