...
def cluster_umap(fcs_df, rp, clustering_dir, label, groupby, figure_dir):
    ...
    image_scale = 3
    fig, axes = plt.subplots(1, 1, figsize=(4.5, 4.5))  # 1 row, 2 columns
    ...
    for ii in index_set:#set(partition_df['partition']):
        im=axes.scatter(df.loc[df[groupby]==ii,'ux'], \
                    ...
                    s = 0.05, label = ii, c = color_list[c_idx], alpha = 0.01)
        axes.set_title(f'{groupby}_rp = {rp}')
        c_idx = c_idx + 1
    
    axes.axis('equal');
    legend = plt.legend(markerscale=20, bbox_to_anchor=(1.05, 1), loc='upper left')
    ...
    for partition_id in range(len(index_set)):
        ...
        for ii in index_set:
            ...
            im=axes[ai,aj].scatter(df.loc[df[groupby]==ii,'ux'], \
                    df.loc[df[groupby]==ii,'uy'], \
                    s = 0.1, label = ii, c = "red" if ii==index_set[partition_id] else "green", \
                                   alpha = 0.01 if ii==index_set[partition_id] else 0.01)
...
################# density plots
def density_plot(fcs_df, type_markers, rp, clustering_dir, label, groupby, figure_dir):
...
if __name__ == "__main__":
    ...
    with open('config.yaml', 'r') as f:
        ...
        cofactor_file = config['parameters']['z04']['cofactorfile']
        rp_list = eval(config['parameters']['z03']['rp_list'])

    figure_dir = 'round_one_clustering'
    if not os.path.isdir(figure_dir):
        os.mkdir(figure_dir)
    clustering_dir=f'partition_{label}'
    
    cofactor_df=pd.read_csv(cofactor_file)
    cofactor_df = cofactor_df.sort_values(by='class')
    type_markers = list(cofactor_df['antigen'][cofactor_df['class']=="clustering"])


    fcs_df = pd.read_csv("fcs_data_umap.csv.gz", compression='gzip')

    # rp = 0.1
    
    # cluster_umap(fcs_df, rp, clustering_dir, label, 'partition', figure_dir)
    # one_vs_others_umap(fcs_df, rp, clustering_dir, label, 'partition', figure_dir)
    # density_plot(fcs_df, type_markers, rp, clustering_dir, label, 'partition', figure_dir)
    ...
    with multiprocessing.get_context('spawn').Pool(4, initializer=init_pool, initargs=(results,)) as pool:
        list(tqdm(pool.imap_unordered(unpack_and_run2, inputs), total=len(inputs)))

    inputs = [(fcs_df, type_markers, rp, clustering_dir, label, 'partition', figure_dir) for rp in rp_list]
    
    with multiprocessing.get_context('spawn').Pool(4, initializer=init_pool, initargs=(results,)) as pool:
        list(tqdm(pool.imap_unordered(unpack_and_run3, inputs), total=len(inputs)))