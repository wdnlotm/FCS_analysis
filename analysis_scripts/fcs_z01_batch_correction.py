...
if __name__ == "__main__":
    ...
    if args.useyaml:
        with open('config.yaml', 'r') as f:
            ...
            figure_dir = outputdir+'/'+config['z01']['figure_dir'] # figure_dir: where to save figures
            fcstrimmed = outputdir+'/'+config['z01']['fcstrimmed']       # data location
            cofactorfile = inputfiles+'/'+config['z01']['cofactorfile']
            numpool =  config['z01']['numpool']
            batchinfo = inputfiles+'/'+config['z01']['batchinfo']
    ...
    with open(f"{outputdir}/markers_to_batchcorrect.csv", "w") as file:
        for item in markers:
            file.write(item + "\n")
    ...
    start_time = time.perf_counter()
    fcs_df_trimmed_transformed_w_custom_cf.to_csv(f"{outputdir}/fcs_df_ready_for_cycombine.csv.gz", index=False, compression='gzip')
    duration = time.perf_counter() - start_time
    print(f"Computation time: {duration:.4f} seconds")
    ...
    cycombine_commands = f"""module load apptainer; SIF=/project/iprime_storage/myles_kim/imageD/R441_cytof/rstudio_R443_v6.sif; 
    apptainer exec $SIF Rscript analysis_scripts/fcs_z01_cyCombine_simple.r {outputdir}/fcs_df_ready_for_cycombine.csv.gz {outputdir}/markers_to_batchcorrect.csv {outputdir}/fcs_df_cycombine_BCed.csv.gz"""
    ...
    fcs_df_BCed = pd.read_csv(f'{outputdir}/fcs_df_cycombine_BCed.csv.gz', compression='gzip')
